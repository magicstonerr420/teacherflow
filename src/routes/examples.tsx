...
import { previewSection, type SectionKey } from '@/lib/lesson-sections';
import { PreviewNavigation } from '@/components/lesson/PreviewNavigation';
...
export const Route=createFileRoute('/examples')({
  validateSearch:(search:Record<string,unknown>):{lesson?:string;section?:SectionKey}=>({
    ...(typeof search['lesson']==='string'?{lesson:search['lesson']}:{}),
    ...(search['section'] ? {section:previewSection(search['section'])} : {}),
  }),
...
function Examples(){
  const {lesson:slug,section='overview'}=Route.useSearch();
  ...
  return <AppShell><div className="mx-auto max-w-6xl px-5 py-10 sm:py-14">
    {slug ? <><Link to="/examples" search={{}} className="mb-6 inline-block font-medium text-primary underline">← Browse all previews</Link>
      {preview && <PreviewNavigation preview={preview} section={section}/>}
      {!preview?<div role="alert" className="rounded-xl border p-6">That preview does not exist. Choose one of the eight lessons from the gallery.</div>
       ...
       :loaded?.slug===preview.slug?<Suspense fallback={<p role="status">Opening lesson materials…</p>}><PreviewLesson key={preview.slug} data={loaded.data} preview={preview} section={section}/></Suspense>:<p role="status">Loading {preview.title}…</p>}
    </>:<>
      ...
      <Button asChild size="lg" className="mb-8"><Link to="/examples" search={{lesson:FIRST_PREVIEW_SLUG}}>Start with the beginner lesson<ArrowRight className="size-4"/></Link></Button>
      <Button asChild variant="outline" size="lg" className="mb-8 sm:ml-3"><Link to="/request-access">Request beta access</Link></Button>
