...
export async function generateListening(request: LessonRequestInput, lesson: Partial<LessonPackage>, scope: string, limited = false): Promise<ListeningState> {
  ...
  const id = hash(stable({ scope, context, system: LISTENING_SYSTEM, model: 'deepseek/deepseek-v4-flash-0731' }));
  const result = await cached<ListeningState>(id, scope, limited, async () => {
    ...
    const validate = (v: unknown) => validateListening(americanEnglishContent(v), request.level, request.studentAge);
    try { validate(value); }
    catch (error) {
  ...
  return result.status === 'ready' ? { ...result, value: validateListening(result.value, request.level, request.studentAge) } : result;
}
