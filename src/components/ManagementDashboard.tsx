...
const tabs = [ ['overview', 'Overview'], ['teachers', 'Teachers'], ['activity', 'Generation activity'], ['issues', 'Issues'], ['budget', 'Budget'], ['feedback', 'Feedback'] ] as const;
...
function OperationDetails({ row, data, onAction }: { row: Operation; data: ManagementData; onAction: (action: Confirmation) => void }) {
  ...
  return <details className="group rounded-xl border bg-card" aria-label={`${topic(row.request)} — ${partName(row.part)}`}>
    ...
    <div className="space-y-4 border-t p-4 text-sm">
      ...
      <details className="rounded-lg border p-3"><summary className="cursor-pointer font-medium">Provider attempts ({row.providers.length})</summary>{row.providers.length ? <ol className="mt-3 space-y-3">{row.providers.map((attempt, i) => <li key={`${attempt.at}-${i}`} className="space-y-1 rounded-md bg-muted/50 p-3"><p className="break-words font-medium">{i + 1}. {attempt.model} · {attempt.kind}</p><p className="text-xs text-muted-foreground">{when(attempt.at)} · {(attempt.ms / 1000).toFixed(1)} seconds · {attempt.status ? `HTTP ${attempt.status}` : 'HTTP status not recorded'}</p>{attempt.detail && <p className="whitespace-pre-wrap break-words">{attempt.detail}</p>}</li>)}</ol> : <p className="mt-3 text-muted-foreground">No provider attempt details were recorded. This is not evidence that no request or charge occurred.</p>}</details>
...
export function ManagementDashboard() {
  ...
  const tab = hash === 'lesson-allowances' ? 'teachers' : tabs.some(([value]) => value === hash) ? hash : 'overview';
  ...
  return <div className="min-w-0 space-y-4">
    ...
    {query.isError && <p role="alert" className="rounded-lg border border-destructive p-3 text-sm">Could not refresh Management. {data ? 'The last loaded records are still shown. ' : ''}Please try Refresh dashboard.</p>}
    ...
    <Tabs value={tab} onValueChange={value => { tabClick.current = true; void navigate({ to: '/beta-management', hash: value, replace: true, resetScroll: false, hashScrollIntoView: false }); }} className="min-w-0 space-y-5">
      <TabsList aria-label="Management sections" className="grid h-auto w-full grid-cols-3 gap-1 p-1.5 lg:grid-cols-6">{tabs.map(([value, label]) => <TabsTrigger key={value} value={value} className="min-h-10 whitespace-normal px-2 text-center text-xs sm:text-sm">{label}{value === 'issues' && openIssues.length > 0 && <span className="ml-1.5 rounded-full bg-background/80 px-1.5 text-xs">{openIssues.length}</span>}</TabsTrigger>)}</TabsList>
