...
export function AccessRequestsPanel() {
  ...
  return <section className="space-y-5 rounded-xl border bg-card p-4 sm:p-5" aria-label="Beta access requests">
    ...
    {query.data?.email && <p className="rounded-lg bg-muted p-3 text-sm">{query.data.email.configured ? 'Approval emails are enabled. The queue is checked every minute. “Accepted by provider” confirms submission to Resend, not inbox delivery.' : 'Email setup needed: add RESEND_API_KEY and a verified sender in TEACHERFLOW_ALERT_FROM in Render. New approval emails stay queued until setup is complete. Earlier approvals are not emailed retroactively.'}</p>}
