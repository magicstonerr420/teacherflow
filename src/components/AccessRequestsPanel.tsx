...
export function AccessRequestsPanel() {
  ...
  async function decide(row: AccessRequestRow, decision: 'approved' | 'declined') {
    ...
    try {
      ...
      setMessage(decision === 'approved' ? `${row.name} is approved. They can sign in as ${row.email} and use Check request status to activate their three-lesson invitation. ${query.data?.email.configured ? 'Approval email queued; check the Approved list for its status.' : 'The email is waiting for sender setup.'}` : `${row.name}'s request was declined. No invitation was created.`);
  ...
  return <section className="space-y-5 rounded-xl border bg-card p-4 sm:p-5" aria-label="Beta access requests">
    ...
    <p className="text-sm text-muted-foreground">Approval reserves an invitation for the applicant's email. They activate it after signing in. Each teacher has three lesson slots, and your shared beta budget stays in place.</p>
    {query.data?.email && <p className="rounded-lg bg-muted p-3 text-sm">{query.data.email.configured ? `Approval email sender configured: ${query.data.email.provider === 'gmail-script' ? 'Gmail' : 'Resend'}. The queue is checked every minute while the service is running. “Accepted by provider” confirms submission, not inbox delivery.` : 'Email setup needed: connect a Gmail sender or a verified Resend sender in the server settings. New approval emails wait until setup is complete. Earlier approvals are not emailed retroactively.'}</p>}
    ...
    {query.isPending ? <p role="status">Loading requests…</p> : query.isError ? <p role="alert">Could not load requests. Use Refresh requests to try again.</p> : <>
      ...
      <div className="space-y-4">{query.data.requests.map(row => <article key={row.id} aria-label={`Request from ${row.name}`} className="space-y-3 rounded-lg border p-4"><div className="flex flex-wrap items-start justify-between gap-2"><h3 className="break-words font-semibold">{row.name}</h3><span className="rounded-full bg-muted px-2 py-1 text-xs capitalize">{row.status}</span></div><p className="break-all text-sm">{row.email}</p><p className="whitespace-pre-wrap break-words text-sm leading-relaxed">{row.teaching}</p><p className="text-xs text-muted-foreground">Requested {new Date(row.created).toLocaleString()}{row.reviewed ? ` · Reviewed ${new Date(row.reviewed).toLocaleString()}` : ''}</p>{row.status === 'approved' && <p className="text-sm"><strong>Approval email: </strong>{row.emailStatus === 'accepted' ? 'Accepted by provider' : row.emailStatus === 'queued' ? query.data.email.configured ? 'Queued for sending' : 'Waiting for sender setup' : row.emailStatus === 'review' ? 'Needs review' : row.emailStatus === 'canceled' ? 'Canceled' : 'No email queued (earlier approval)'}{row.emailError && <span className="mt-1 block text-muted-foreground">{row.emailError}</span>}</p>}{row.status === 'pending' && <div className="flex flex-wrap gap-2"><Button size="sm" disabled={busy} onClick={() => void decide(row, 'approved')}>Approve — 3 lessons</Button><Button size="sm" variant="outline" disabled={busy} onClick={() => void decide(row, 'declined')}>Decline request</Button></div>}</article>)}</div>
