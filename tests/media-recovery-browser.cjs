...
(async()=>{
 ...
 try{
  const page=await browser.newPage({viewport:{width:1440,height:1000}}),errors=[],unexpected=[],events=[];
  page.on('pageerror',e=>errors.push(e.message));
  await page.routeWebSocket(/.*/,s=>s.close());
  await page.route('**/api/generation-events',r=>{events.push(r.request().postDataJSON());return r.fulfill({json:{ok:true}});});
  ...
  assert.ok(events.some(e=>e.part==='recording'&&e.status==='completed'),'Recording recovery is reported automatically');
  assert.ok(events.some(e=>e.part==='reading'&&e.status==='failed'),'Exhausted reading recovery is reported automatically');
  assert.ok(events.every(e=>!e.error?.includes('<!DOCTYPE')),'Gateway HTML is excluded from diagnostic reports');
