...
(async () => {
  const directory = await fs.mkdtemp(path.resolve('.local-runtime/access-request-qa-'));
  let project;
  ...
  const server = spawn(process.execPath, ['--experimental-transform-types', 'tests/access-request-runtime-server.mjs'], {windowsHide: true, env: {...process.env, ACCESS_REQUEST_FIXTURE: directory}, stdio: ['ignore', 'pipe', 'pipe']});
  ...
  try {
    ...
    async function client(id, email, mobile = false) {
      ...
      if (id) await context.addInitScript(({project, id, email}) => localStorage.setItem(`sb-${project}-auth-token`, JSON.stringify({access_token: id, refresh_token: 'fixture-refresh', expires_at: Math.floor(Date.now()/1000)+3600, expires_in: 3600, token_type: 'bearer', user: {id, email, app_metadata: {}, user_metadata: {}}})), {project, id, email});
