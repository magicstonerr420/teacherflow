...
(async () => {
  ...
  try {
    ...
    console.log(JSON.stringify({ passed: true, checks: ['six sections', 'sanitized gateway and credential diagnostics', 'stale-data preservation', 'failed first load and refresh recovery', 'unverified access privacy', 'email preferences and setup', 'delivery status', 'submitted settings', 'provider attempts', 'targeted retry confirmation', 'duplicate action guard', 'unfinished slot return', 'unsafe recovery exclusions', 'filtering', 'private notes', 'legacy history', 'existing invitations and feedback', 'budget', 'mobile layout', 'legacy hash navigation', 'owner isolation'], paidRequests: 0 }));
