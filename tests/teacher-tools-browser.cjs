...
(async()=>{
  ...
  try {
    ...
    await module('**/src/lib/management.functions.ts*',`
      export const loadManagement=async()=>({operations:[],teachers:[],lessons:[],notes:[],audit:[],history:[],budget:{limitUsd:10,remainingUsd:9,accountedUsd:1,reservedUsd:0,paused:false},spending:[],settings:{emailEnabled:false,dailySummary:false},email:{configured:false,to:'owner@example.test',missing:['RESEND_API_KEY','TEACHERFLOW_ALERT_FROM']},alerts:[]});
    ...
    await mount('/beta-management#feedback','owner');
    ...
    await nav.getByRole('button',{name:'Management',exact:true}).click();
    const admin=page.getByRole('menu',{name:'Management',exact:true});
    await admin.getByRole('menuitem',{name:'Feedback',exact:true}).click();
    ...
    await nav.getByRole('button',{name:'Management',exact:true}).click();
    await admin.getByRole('menuitem',{name:'Budget',exact:true}).click();
    ...
    await drawer.getByRole('heading',{name:'Management',exact:true}).waitFor();
    ...
    await mount('/about','teacher');
    await page.getByRole('button',{name:'Menu',exact:true}).click();
    assert.equal(await drawer.getByRole('heading',{name:'Management',exact:true}).count(),0);
    ...
    console.log('PASS: teacher tools, All lessons/Favorites tabs, guest/teacher/owner navigation, keyboard menus, password link, Management anchors, mobile menu and focus, sign-out failure/retry; no live writes or AI calls.');
